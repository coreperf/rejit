...
void NB_Codegen::VisitBracket(Bracket* bracket) {
  ...
  if (!(bracket->flags() & Bracket::BracketFlag::non_matching)) {
    __ j(zero, &out);
  } else {
    __ j(not_zero, &out);
  }
...
void NB_FastForwardGen::Generate() {
  ...
  if (regexp_list_->size() == 1) {
    ...
    if (CpuFeatures::IsSupported(SSE4_2) && mc_only &&
        regexp_list_->size() < 10) {
      uint8_t pcmp_str_control =
        Assembler::unsigned_bytes | Assembler::equal_order |
        Assembler::pol_pos | Assembler::lsi;
      Label simd_loop, maybe_match, keep_searching, find_null, exit;
      static const int xmm_cb = 4;

      for (unsigned i = 0; i < regexp_list_->size(); i++) {
        __ movdqp(XMMRegister::from_code(xmm_cb + i),
      ...
      __ movdqu(xmm0, Operand(rax, 0));

      for (unsigned i = 0; i < regexp_list_->size(); i++) {
        __ pcmpistri(pcmp_str_control, XMMRegister::from_code(xmm_cb + i), xmm0);
...
void NB_FastForwardGen::VisitSingleMultipleChar(MultipleChar* mc) {
  ...
  if (CpuFeatures::IsSupported(SSE4_2)) {
    ...
    // Interesting flags after a pcmpistri
    //  CF == 1 iff there is a submatch.
    //  ZF == 1 iff the src operand contains a null char.

    __ movdqu(xmm1, Operand(rax, 0));

    __ bind(&simd_loop);

    // TODO(rames): doc indicates return mask in xmm0. check about that.
...
//  } else {
    ...
    Label* target =
      bracket->flags() & Bracket::BracketFlag::non_matching ? &done : &match;
...
void NB_FastForwardGen::VisitSingleStartOfLine(StartOfLine* sol) {
  if (CpuFeatures::IsSupported(SSE4_2)) {
    Label simd_loop, adjust, found, exit;
    Label p1, p2, p3;
    uint8_t pcmp_str_control =
      Assembler::unsigned_bytes | Assembler::equal_any |
      Assembler::pol_pos | Assembler::lsi;
...
void NB_FastForwardGen_iter::VisitSingleBracket(Bracket* bracket) {
  ...
  if (single_next) {
    ...
    Label advance;
    Label* target = bracket->flags() & Bracket::BracketFlag::non_matching ? &advance : &done;
    __ subq(string_pointer, Immediate(kCharSize));
    __ Move(scratch1, 0);
    __ Move(scratch2, 0);
    __ Move(scratch3, 0);
